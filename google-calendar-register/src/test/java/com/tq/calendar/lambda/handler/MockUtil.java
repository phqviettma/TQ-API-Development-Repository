...
public class MockUtil {
	public static Env mockEnv() {
		...
		when(env.getAwsSecretAccessKey()).thenReturn("qqEpFbGX+/ePBkvrBsHK3hsTqaRhi0qr6aLhUXTK");
		when(env.getAwsAccessKeyId()).thenReturn("AKIAIHW6IKOWVUOZRQJA");
		when(env.getSimplyBookCompanyLogin()).thenReturn("trancanh");
		when(env.getSimplyBookUser()).thenReturn("admin");
		when(env.getSimplyBookPassword()).thenReturn("unYpE7U7Es");
